...
void for_each_pair_empty()
{
	...
	int n = 0;
	esu::for_each_pair(v.begin(), v.end(), [&n](...) { ++n; });
	assert(n == 0);
}
...
void for_each_pair_two()
{
	...
	esu::for_each_pair(v.begin(), v.end(), [&o](int x, int y) { o.push_back({x, y}); });
...
void for_each_pair_n_negative_zero()
{
	...
	int n = 0;
	esu::for_each_pair_n(v.begin(), -1, [&n](...) { ++n; });
	assert(n == 0);
}
...
void for_each_pair_n_zero()
{
	...
	int n = 0;
	esu::for_each_pair_n(v.begin(), 0, [&n](...) { ++n; });
	assert(n == 0);
}
...
void for_each_pair_n_main()
{
	...
	esu::for_each_pair_n(v.begin(), 4, [&o](int x, int y) { o.push_back({x, y}); });
	...
	o.clear();
	esu::for_each_pair_n(v.begin() + 1, 3, [&o](int x, int y) { o.push_back({x, y}); });
